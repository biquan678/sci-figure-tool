...
export default function JournalPage(){
  ...
  const j = journals.find(x => x.name === 'Cell Research');
  ...
  useEffect(() => {
    if (canvasRef.current) {
      renderTemplate({ name: 'Cell Research', widthMm, heightMm: height, dpi, label: mode, target: canvasRef.current });
    }
    if (open && modalRef.current) {
      renderTemplate({ name: 'Cell Research', widthMm, heightMm: height, dpi, label: mode, target: modalRef.current });
    }
  }, [mode, widthMm, height, dpi, open]);

  const handleDownload = (label: string, widthMm: number) => {
    downloadTemplate({ name: 'Cell Research', widthMm, heightMm: height, dpi, label, canvas: canvasRef.current || undefined });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <SEO title="Cell Research Figure Requirements" description="Figure size, DPI, and format requirements for Cell Research." />
      <h1 className="text-3xl font-bold">{t('journal.title', { name: 'Cell Research' })}</h1>
