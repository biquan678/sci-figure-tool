...
export default function JournalPage(){
  ...
  const j = journals.find(x => x.name === 'Global Change Biology');
  ...
  useEffect(() => {
    if (canvasRef.current) {
      renderTemplate({ name: 'Global Change Biology', widthMm, heightMm: height, dpi, label: mode, target: canvasRef.current });
    }
    if (open && modalRef.current) {
      renderTemplate({ name: 'Global Change Biology', widthMm, heightMm: height, dpi, label: mode, target: modalRef.current });
    }
  }, [mode, widthMm, height, dpi, open]);

  const handleDownload = (label: string, widthMm: number) => {
    downloadTemplate({ name: 'Global Change Biology', widthMm, heightMm: height, dpi, label, canvas: canvasRef.current || undefined });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <SEO title="Global Change Biology Figure Requirements" description="Figure size, DPI, and format requirements for Global Change Biology." />
      <h1 className="text-3xl font-bold">{t('journal.title', { name: 'Global Change Biology' })}</h1>
