...
export default function JournalPage(){
  ...
  const j = journals.find(x => x.name === 'Cambridge University Press');
  ...
  useEffect(() => {
    if (canvasRef.current) {
      renderTemplate({ name: 'Cambridge University Press', widthMm, heightMm: height, dpi, label: mode, target: canvasRef.current });
    }
    if (open && modalRef.current) {
      renderTemplate({ name: 'Cambridge University Press', widthMm, heightMm: height, dpi, label: mode, target: modalRef.current });
    }
  }, [mode, widthMm, height, dpi, open]);

  const handleDownload = (label: string, widthMm: number) => {
    downloadTemplate({ name: 'Cambridge University Press', widthMm, heightMm: height, dpi, label, canvas: canvasRef.current || undefined });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <SEO title="Cambridge University Press Figure Requirements" description="Figure size, DPI, and format requirements for Cambridge University Press." />
      <h1 className="text-3xl font-bold">{t('journal.title', { name: 'Cambridge University Press' })}</h1>
