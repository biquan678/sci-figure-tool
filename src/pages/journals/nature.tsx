...
export default function JournalPage(){
  ...
  const j = journals.find(x => x.name === 'Nature');
  const dpi = j?.minDPI || 300;
  const single = j?.singleColumnWidth || 89;
  const double = j?.doubleColumnWidth || 183;
  const height = j?.maxHeight || 247;
  ...
  useEffect(() => {
    if (canvasRef.current) {
      renderTemplate({ name: 'Nature', widthMm, heightMm: height, dpi, label: mode, target: canvasRef.current });
    }
    if (open && modalRef.current) {
      renderTemplate({ name: 'Nature', widthMm, heightMm: height, dpi, label: mode, target: modalRef.current });
    }
  }, [mode, widthMm, height, dpi, open]);

  const handleDownload = (label: string, widthMm: number) => {
    downloadTemplate({ name: 'Nature', widthMm, heightMm: height, dpi, label, canvas: canvasRef.current || undefined });
  };
