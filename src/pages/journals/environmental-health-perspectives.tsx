...
export default function JournalPage(){
  ...
  const j = journals.find(x => x.name === 'Environmental Health Perspectives');
  ...
  useEffect(() => {
    if (canvasRef.current) {
      renderTemplate({ name: 'Environmental Health Perspectives', widthMm, heightMm: height, dpi, label: mode, target: canvasRef.current });
    }
    if (open && modalRef.current) {
      renderTemplate({ name: 'Environmental Health Perspectives', widthMm, heightMm: height, dpi, label: mode, target: modalRef.current });
    }
  }, [mode, widthMm, height, dpi, open]);

  const handleDownload = (label: string, widthMm: number) => {
    downloadTemplate({ name: 'Environmental Health Perspectives', widthMm, heightMm: height, dpi, label, canvas: canvasRef.current || undefined });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <SEO title="Environmental Health Perspectives Figure Requirements" description="Figure size, DPI, and format requirements for Environmental Health Perspectives." />
      <h1 className="text-3xl font-bold">{t('journal.title', { name: 'Environmental Health Perspectives' })}</h1>
