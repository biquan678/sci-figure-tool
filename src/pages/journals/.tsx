...
export default function JournalPage(){
  ...
  const j = journals.find(x => x.name === '中国药理学通报');
  ...
  useEffect(() => {
    if (canvasRef.current) {
      renderTemplate({ name: '中国药理学通报', widthMm, heightMm: height, dpi, label: mode, target: canvasRef.current });
    }
    if (open && modalRef.current) {
      renderTemplate({ name: '中国药理学通报', widthMm, heightMm: height, dpi, label: mode, target: modalRef.current });
    }
  }, [mode, widthMm, height, dpi, open]);

  const handleDownload = (label: string, widthMm: number) => {
    downloadTemplate({ name: '中国药理学通报', widthMm, heightMm: height, dpi, label, canvas: canvasRef.current || undefined });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <SEO title="中国药理学通报 Figure Requirements" description="Figure size, DPI, and format requirements for 中国药理学通报." />
      <h1 className="text-3xl font-bold">{t('journal.title', { name: '中国药理学通报' })}</h1>
