...
export default function JournalPage(){
  ...
  const j = journals.find(x => x.name === 'Nature Medicine');
  ...
  useEffect(() => {
    if (canvasRef.current) {
      renderTemplate({ name: 'Nature Medicine', widthMm, heightMm: height, dpi, label: mode, target: canvasRef.current });
    }
    if (open && modalRef.current) {
      renderTemplate({ name: 'Nature Medicine', widthMm, heightMm: height, dpi, label: mode, target: modalRef.current });
    }
  }, [mode, widthMm, height, dpi, open]);

  const handleDownload = (label: string, widthMm: number) => {
    downloadTemplate({ name: 'Nature Medicine', widthMm, heightMm: height, dpi, label, canvas: canvasRef.current || undefined });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <SEO title="Nature Medicine Figure Requirements" description="Figure size, DPI, and format requirements for Nature Medicine." />
      <h1 className="text-3xl font-bold">{t('journal.title', { name: 'Nature Medicine' })}</h1>
