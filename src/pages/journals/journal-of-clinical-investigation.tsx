...
export default function JournalPage(){
  ...
  const j = journals.find(x => x.name === 'Journal of Clinical Investigation');
  ...
  useEffect(() => {
    if (canvasRef.current) {
      renderTemplate({ name: 'Journal of Clinical Investigation', widthMm, heightMm: height, dpi, label: mode, target: canvasRef.current });
    }
    if (open && modalRef.current) {
      renderTemplate({ name: 'Journal of Clinical Investigation', widthMm, heightMm: height, dpi, label: mode, target: modalRef.current });
    }
  }, [mode, widthMm, height, dpi, open]);

  const handleDownload = (label: string, widthMm: number) => {
    downloadTemplate({ name: 'Journal of Clinical Investigation', widthMm, heightMm: height, dpi, label, canvas: canvasRef.current || undefined });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <SEO title="Journal of Clinical Investigation Figure Requirements" description="Figure size, DPI, and format requirements for Journal of Clinical Investigation." />
      <h1 className="text-3xl font-bold">{t('journal.title', { name: 'Journal of Clinical Investigation' })}</h1>
