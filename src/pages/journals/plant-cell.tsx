...
export default function JournalPage(){
  ...
  const j = journals.find(x => x.name === 'Plant Cell');
  ...
  useEffect(() => {
    if (canvasRef.current) {
      renderTemplate({ name: 'Plant Cell', widthMm, heightMm: height, dpi, label: mode, target: canvasRef.current });
    }
    if (open && modalRef.current) {
      renderTemplate({ name: 'Plant Cell', widthMm, heightMm: height, dpi, label: mode, target: modalRef.current });
    }
  }, [mode, widthMm, height, dpi, open]);

  const handleDownload = (label: string, widthMm: number) => {
    downloadTemplate({ name: 'Plant Cell', widthMm, heightMm: height, dpi, label, canvas: canvasRef.current || undefined });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <SEO title="Plant Cell Figure Requirements" description="Figure size, DPI, and format requirements for Plant Cell." />
      <h1 className="text-3xl font-bold">{t('journal.title', { name: 'Plant Cell' })}</h1>
