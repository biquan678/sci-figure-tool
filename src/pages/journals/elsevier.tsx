...
export default function JournalPage(){
  ...
  useEffect(() => {
    if (canvasRef.current) {
      renderTemplate({ name: 'Elsevier', widthMm, heightMm: height, dpi, label: mode, target: canvasRef.current });
    }
    if (open && modalRef.current) {
      renderTemplate({ name: 'Elsevier', widthMm, heightMm: height, dpi, label: mode, target: modalRef.current });
    }
  }, [mode, widthMm, height, dpi, open]);

  const handleDownload = (label: string, widthMm: number) => {
    downloadTemplate({ name: 'Elsevier', widthMm, heightMm: height, dpi, label, canvas: canvasRef.current || undefined });
  };
