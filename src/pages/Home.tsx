...
export default function Home() {
  ...
  const filteredPosts = posts.filter(p => p.lang === locale || (locale !== 'zh' && locale !== 'ja' && p.lang === 'en')).slice(0, 6);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl mx-auto mb-12">
        <Link to={localizePath('/figure-checker', locale)} className="group p-6 bg-white rounded-xl border border-gray-200 hover:border-blue-400 hover:shadow-md transition-all">
        ...
        <Link to={localizePath('/image-converter', locale)} className="group p-6 bg-white rounded-xl border border-gray-200 hover:border-blue-400 hover:shadow-md transition-all">
          <div className="text-3xl mb-3">🔄</div>
      ...
      {filteredPosts.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to={localizePath('/blog', locale)} className="text-sm text-blue-600 hover:underline">{t('nav.blog')} →</Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPosts.map(post => (
              <Link to={localizePath(`/blog/${post.slug}`, locale)} key={post.slug} className="block bg-white rounded-lg border border-gray-200 p-5 hover:shadow-md transition-shadow">
