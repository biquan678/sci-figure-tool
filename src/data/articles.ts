...
論文図の準備は、SCI投稿における最重要ステップの一つです。図の形式が整っていないだけで、原稿が差し戻しや修正対象になることは珍しくありません。
...
現在は多くのジャーナルでRGBが標準です。CMYKは印刷向けであり、オンライン出版中心の投稿にはあまり向きません。

## 4. フォント

- Nature、Science、Cell：Helvetica または Arial
- 最小サイズ：5〜6pt
...
- 単一カラム：80〜90mm
- ダブルカラム：170〜183mm
- 最終印刷サイズを想定してデザインすること

## 6. ファイルサイズ

多くのジャーナルでは、1ファイルあたり10MB以下が目安です。TIFFではLZW圧縮を使うと効率的です。
...
[図チェッカー](/figure-checker)で自動チェックできます。
`,

'tiff-conversion-guide-ja': `
## TIFF形式の重要性

多くの学術誌がTIFF形式を要求します。TIFFには次の利点があります。

- **無劣化圧縮** — LZW圧縮なら画質を保ったまま保存可能
- **業界標準** — 印刷・出版で広く使われる形式
- **メタデータ対応** — DPIや色空間などの情報を埋め込める
...
メリット：ブラウザ上で完結し、サーバーへのアップロードも不要です。

### Photoshopの場合

1. 画像を開く：File → Open
2. 解像度設定：Image → Image Size → Resolution を 300 に設定
...
- 拡張子だけを変更する（.jpg → .tiff）
- 低解像度画像を無理にアップスケールする
- 機密性の高い画像を不用意にオンラインへアップロードする

### ✅ 正しい方法
- 元ファイル（AI / PSD / R / Python）から直接TIFFを書き出す
- 元の解像度を維持し、無理に拡大しない
- LZW圧縮を使う
- 変換後は[図チェッカー](/figure-checker)で確認する

## よくある問題

**Q: TIFFファイルが大きすぎる**
A: LZW圧縮を使えば通常30〜50%ほど小さくできます。それでも大きい場合は、DPI設定を確認してください。

**Q: 変換後に色が変わって見える**
A: カラーモードを確認してください。基本的には RGB → RGB のまま変換するのが安全です。

**Q: ジャーナルから「受け入れられない」と言われた**
A: 圧縮方式が原因の可能性があります。JPEG圧縮のTIFFではなく、LZW圧縮のTIFFを試してください。
`,
...
[図チェッカー](/figure-checker)でジャーナル名を選んで図をアップロードすれば、主要要件を自動でチェックできます。
`,
